#*******************************************
# ENERGYPLUS PLUGIN: MANUAL LIGHTING CONTROL
#*******************************************
...
class LightingControl(EnergyPlusPlugin): 
    ...
    def __init__(self): 
        ...
        directory_data = r'' # Give the directory in which the data is saved. 
    ...
    def on_begin_timestep_before_predictor(self,state) -> int: 
        ...
        for zone_name in self.rooms_lighting: 
            ...
            previousstate_lighting['PreviousStateLighting{}'.format(zone_name)] = self.api.exchange.get_global_value(state, self.handle_previouslighting[zone_name])
            if zone_name in self.rooms_daylighting:
                illuminance['Illuminance{}'.format(zone_name)] = self.api.exchange.get_variable_value(state, self.handle_illuminance[zone_name])
            elif zone_name in self.rooms_daylighting_2: # Average of two Daylighting Reference Points
                illuminance['Illuminance{}'.format(zone_name)] = (self.api.exchange.get_variable_value(state, self.handle_illuminance[zone_name+"_1"])+self.api.exchange.get_variable_value(state, self.handle_illuminance[zone_name+"_2"]))/2
